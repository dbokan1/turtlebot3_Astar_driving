...
const int IDIM = 109; // horizontal size of the squares
const int JDIM = 109; // vertical size size of the squares
...
string pathFind( const Location &locStart, const Location &locFinish) {
    ...
    while(!q[qi].empty()) {
        ...
        for(i = 0; i < NDIR; i++) {
            ...
            if(!(iNext < 0 || iNext > IDIM - 1 || jNext < 0 || jNext > JDIM - 1 || /*squares[iNext][jNext]*/ map.at<float>(iNext,jNext) == 1 || closedNodes[iNext][jNext] == 1)) {
...
int main()
{
    map = cv::imread("D:\\test.jpg", cv::IMREAD_COLOR);
    cv::threshold(map, map, 95, 255, THRESH_BINARY);
